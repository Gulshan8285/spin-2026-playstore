 ...
 */
export const verifyQuiz = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "User must be authenticated.");
  }
  const uid = context.auth.uid;
 ...
 */
export const admobRewardCallback = functions.https.onRequest(async (req, res) => {
...
export const adminProcessWithdrawal = functions.https.onCall(async (data, context) => {
  ...
  if (!wdDoc.exists) throw new functions.https.HttpsError("not-found", "Withdrawal request not found.");
